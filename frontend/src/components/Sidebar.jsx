  ...
  return (
    <div className="hidden sm:flex z-10 w-[13%] flex-col justify-between items-center h-full bg-gray-700 py-5 border-r-2 border-gray-500">
      ...
      <div className="w-full flex flex-col justify-between items-center h-full bg-gray-700 py-5">
          ...
          {tabs.map((tab, index) => (
            ...
            >
              <div className="flex gap-x-2 items-center">
                {tab.name === "Profile" ? (
                  <img src={userimg} alt="" className="w-5 rounded-full" />
                ) : (
                  tab.icon
                )}
                {tab.name}
        ...
        <div className="flex flex-col text-teal-300 font-semibold justify-center items-start text-sm sm:text-md">
          <p className="hover:text-teal-100 cursor-pointer" onClick={() => navigate("/changepassword")}>
            Change password
          </p>

          <div className="hover:text-white cursor-pointer flex px-1 py-2 items-center gap-x-2" onClick={() => setModal(true)}>
            <MdLogout />
            <p>Log out</p>
          </div>
        </div>
